...
class ScaledDotProductAttention(nn.Module):
    ...
    def __init__(self, d_model, d_k, d_v, h, return_att=False):
        '''
...
class ScaledDotProductAttentionMemory(nn.Module):
    ...
    def forward(self, queries, keys, values, attention_mask=None, attention_weights=None):
        ...
        if attention_mask is not None:
            att[:, :, :, :nk] = att[:, :, :, :nk].masked_fill(attention_mask == 1, -np.inf)
        att = torch.softmax(att, -1)
        out = torch.matmul(att, v).permute(0, 2, 1, 3).contiguous().view(b_s, nq, self.h * self.d_v)  # (b_s, nq, h*d_v)
        out = self.fc_o(out)  # (b_s, nq, d_model)
        return out
...
class MultiHeadAttention(Module):
    ...
    def __init__(self, d_model, d_k, d_v, h, dropout=.1, identity_map_reordering=False, can_be_stateful=False, residual=True, 
                 attention_module=None, attention_module_kwargs=None, return_att=False):
        ...
        if attention_module is not None:
            ...
            self.attention = ScaledDotProductAttention(d_model=d_model, d_k=d_k, d_v=d_v, h=h, return_att=return_att)
        ...
        if self.can_be_stateful:
            self.register_state('running_keys', torch.zeros((1, 0, d_model)))
            self.register_state('running_values', torch.zeros((1, 0, d_model)))

    def forward(self, queries, keys, values, attention_mask=None, attention_weights=None):
        if self.can_be_stateful and self._is_stateful:
            self.running_keys = torch.cat([self.running_keys.to(keys.device), keys], 1)
            keys = self.running_keys

            self.running_values = torch.cat([self.running_values.to(values.device), values], 1)
